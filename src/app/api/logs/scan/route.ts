...
export async function POST(request: NextRequest) {
  try {
    ...
    const member = (await db.execute({
      ...
      WHERE m.card_uid = ?`,
      args: [card_uid],
    })).rows[0] as unknown as Record<string, InValue> | undefined;
    ...
    const existingCheckIn = (await db.execute({
      ...
      args: [memberId, today, memberId, today, memberId, today],
    ...
    if (!existingCheckIn) {
      action = 'CHECK_IN';
      await db.execute({
        sql: `INSERT INTO logs (member_id, card_uid, action, timestamp, duration_seconds)
        VALUES (?, ?, 'CHECK_IN', ?, NULL)`,
        args: [memberId, card_uid, nowIso],
      });
    } else {
      action = 'CHECK_OUT';
      ...
      await db.execute({
        sql: `INSERT INTO logs (member_id, card_uid, action, timestamp, duration_seconds)
        VALUES (?, ?, 'CHECK_OUT', ?, ?)`,
        args: [memberId, card_uid, nowIso, duration_seconds],
      });
    }
